...
func TestValidateAmendAgainstOriginalPreservesHeadSubject(t *testing.T) {
	...
	errs = ValidateAmendAgainstOriginal(original, Shape(`feat(cli): add commit command
...
query helpers and generated accessors.`
	...
	got := Shape(output)
	want := `refactor(uc): adopt typed query helpers across asterisk, IM and phoneconfig
...
func TestShapeDoesNotLeaveWrappedSubjectShardInBody(t *testing.T) {
	t.Parallel()
...
func TestPreparedCommitPreservesCurrentFocusDiff(t *testing.T) {
	...
	got := prepared.RenderForPrompt()
	for _, want := range []string{`"focus_diff": "current focused evidence"`, `"focus_diff_paths"`, `"current.go"`, `"focus_diff_truncated": true`} {
		if !strings.Contains(got, want) {
...
func TestValidateAllowsContentRatherThanMatchingSubstrings(t *testing.T) {
	t.Parallel()
	original := "fix(parser): also accept empty input"
	output := original + "\n\nHandle false values and also preserve the explanation: field."
	if errs := ValidateAmendAgainstOriginal(original, output); len(errs) != 0 {
		t.Fatal(errs)
	}
	for _, preamble := range []string{"Here is the commit message:", "Commit message:", "I would use this message:"} {
		if errs := Validate(ModeNormal, preamble+"\n\nUpdate parser."); len(errs) == 0 {
			t.Fatalf("accepted %q", preamble)
		}
	}
}

func TestValidateBodyWidthAllowsOnlyUnbreakableContent(t *testing.T) {
	t.Parallel()
	token := "https://example.com/" + strings.Repeat("x", 100)
	for _, prefix := range []string{"", "- ", "1. ", "> ", "Refs: ", "  "} {
		output := Shape("fix(parser): document input\n\n" + prefix + token)
		if errs := Validate(ModeNormal, output); len(errs) != 0 {
			t.Fatalf("prefix %q: %v", prefix, errs)
		}
	}
	for _, body := range []string{strings.Repeat("word ", 16), token + " extra", "- " + token + " extra", "Refs: " + token + " extra"} {
		if errs := Validate(ModeNormal, "subject\n\n"+body); len(errs) == 0 {
			t.Fatalf("accepted wrappable line %q", body)
		}
	}
	if errs := Validate(ModeNormal, "subject\n\n"+strings.Repeat("界", 70)); len(errs) != 0 {
		t.Fatal(errs)
	}
}
