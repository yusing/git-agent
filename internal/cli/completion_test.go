...
func fishCompletionCases(refs, remotes, paths []string) []fishCompletionCase {
	...
	add("index subcommands", "git-agent index ", []string{"gc", "migrate", "sync"})
	...
	add("index gc next option", "git-agent index gc ", []string{"--dry-run"})
	add("index gc options", "git-agent index gc --", []string{"--dry-run"})
	add("index gc complete", "git-agent index gc --dry-run --", nil)
	add("index gc duplicate", "git-agent index gc --dry-run --dry-run --", nil)
	add("index gc positional", "git-agent index gc extra --", nil)
